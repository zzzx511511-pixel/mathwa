...
export default async function OwnerTransferDetailsPage({
  params
}: {
  params: { id: string };
}) {
    ...
    >
      <section className="rounded-2xl border border-ink-900/10 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-2xl font-bold text-brand-400">تفاصيل التحويل</h3>
          <Link
            prefetch={false}
            href="/owner/owner-transfers"
        ...
        {error ? (
          ...
          <div className="mt-4 rounded-xl border border-ink-900/10 bg-brand-100 p-4 text-sm text-ink-900/80">
            لا يوجد تحويل بهذا المعرّف أو لا تملك صلاحية عرضه.
          </div>
        ) : (
          <div className="mt-5">
