...
function getAnyId(row: any): string {
  return String(row?.id ?? row?.invoice_id ?? row?.uuid ?? "—");
}
...
export default async function FinanceInvoicesPage({
  ...
  try {
    ...
    errorMessage = e instanceof Error ? e.message : "تعذر جلب الفواتير.";
  }
  ...
  return (
    ...
    >
      <section className="rounded-2xl border border-ink-900/10 bg-white p-6 shadow-sm">
        <h3 className="text-2xl font-bold text-brand-400">الفواتير (invoices)</h3>
        <p className="mt-2 text-ink-900/80">
          عرض الصفحة {page} من سجلات `invoices` حسب سياسات RLS.
        </p>
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-brand-100 text-ink-900/80">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold">invoice_date</th>
                  ...
                  <tr key={getAnyId(row)} className="border-t border-ink-900/5">
                    <td className="px-4 py-3 font-medium text-ink-900/80">
                      <Link
                        ...
                        href={`/finance/invoices/${getAnyId(row)}`}
                      >
                        {getAnyId(row)}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-ink-900/70">
                      {String(row?.invoice_date ?? row?.due_date ?? "—")}
                    </td>
                    <td className="px-4 py-3 text-ink-900/70">
