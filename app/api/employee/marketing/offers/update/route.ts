...
export async function POST(req: Request) {
  ...
  const { data, error } = await db.from("listings").update(row).eq("id", listingIdRaw).select("id").single();
  ...
  if (error) {
    ...
    const hint =
      msg.includes("listings") && (msg.includes("does not exist") || msg.includes("schema cache"))
        ...
        : "";
    console.error("marketing offers update:", error);
    return NextResponse.json(
      {
        ok: false,
        error: `listing_update_failed: ${msg}.${hint}`
      },
      { status: 500 }
    );
  }
