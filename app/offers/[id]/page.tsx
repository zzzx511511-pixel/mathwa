...
import type { ListingRow } from "@/lib/site/marketing-listings-public";
...
function galleryFromListing(raw: unknown): string[] {
  if (!raw) return [];
...
export default async function OfferDetailsPage({
  ...
  if (listingRaw) {
    const L = listingRaw as ListingRow;
    const galleryUrls = galleryFromListing(L.gallery_urls);
    const main =
      (L.main_image_url && String(L.main_image_url).trim()) || galleryUrls[0] || "";
    const video = L.video_url ? String(L.video_url).trim() : "";
    const type = String(L.property_type ?? "عقار");
    const details = String(L.description ?? "لا توجد تفاصيل إضافية.");
    const area = [L.city, L.district].filter(Boolean).join(" - ") || "—";
    const price =
      L.price != null && Number.isFinite(Number(L.price))
        ? `${new Intl.NumberFormat("ar-SA").format(Number(L.price))} ر.س`
        : "السعر عند التواصل";
    const fallbackImage = /مصنع|مستودع|ورشة|صناعي/i.test(type)
      ? suggestedIndustrialImages[0]
      : suggestedGeneralImages[0];
    const mainMedia = main || fallbackImage;
    const isVideoHero = /\.(mp4|webm|ogg)(\?.*)?$/i.test(mainMedia);
    const gallery = [...new Set([mainMedia, ...galleryUrls].filter(Boolean))];
    const extraImages = gallery.filter((u) => !/\.(mp4|webm|ogg)(\?.*)?$/i.test(u));
        ...
        <section className="rounded-2xl border border-ink-900/10 bg-white p-6">
          <div className="mb-4 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h1 className="text-3xl font-extrabold text-ink-900">{L.title ?? "تفاصيل العرض"}</h1>
              <BackButton fallbackHref="/offers" />
            </div>
            <OfferPublicToolbar
              path={`/offers/${id}`}
              title={String(L.title ?? "عرض عقاري")}
              kind="offer"
            />
          </div>
        ...
        <section className="rounded-2xl bg-[#efe8dc] p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
            {extraImages.length > 0 ? (
              ...
              <p className="text-sm text-ink-900/70">لا توجد صور إضافية.</p>
            )}
          </div>
        </section>
