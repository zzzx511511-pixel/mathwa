...
export default async function TenantMaintenancePage({
  ...
  try {
    ...
    errorMessage = e instanceof Error ? e.message : "تعذر جلب طلبات الصيانة.";
  }
  ...
  return (
    ...
    >
      <section className="rounded-2xl border border-ink-900/10 bg-white p-6 shadow-sm">
        <h3 className="text-2xl font-bold text-brand-400">الصيانة (maintenance)</h3>
        <p className="mt-2 text-ink-900/80">
          عرض الصفحة {page} من سجلات `maintenance_requests` حسب سياسات RLS.
        </p>
                  ...
                  <tr key={getAnyId(row)} className="border-t border-ink-900/5">
                    <td className="px-4 py-3 font-medium text-ink-900/80">
                      <Link
                        ...
                        href={`/tenant/maintenance/${getAnyId(row)}`}
                      >
                        {getAnyId(row)}
                      </Link>
                    </td>
